...
files.forEach(file => {
  ...
  // Compress to 720p max, 30fps, CRF 30, ultrafast, strip audio
  const cmd = `"${ffmpeg}" -i "${inputPath}" -vf "scale='min(1280,iw)':-2" -r 30 -c:v libx264 -preset ultrafast -crf 30 -an -y "${outputPath}"`;
  try {
